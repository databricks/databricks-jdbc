...
public class DatabricksConnectionContext implements IDatabricksConnectionContext {
  ...
  public static IDatabricksConnectionContext parse(String url, Properties properties)
    ...
    if (!ValidationUtil.isValidJdbcUrl(url)) {
      throw new DatabricksParsingException("Invalid url " + url, DatabricksDriverErrorCode.CONNECTION_ERROR,null);
    }
      ...
      for (int urlPartIndex = 1; urlPartIndex < urlParts.length; urlPartIndex++) {
        ...
        if (pair[0].equalsIgnoreCase(PORT)) {
          try {
            ...
            throw new DatabricksParsingException("Invalid port number " + pair[1], DatabricksDriverErrorCode.CONNECTION_ERROR,null);
          }
        }
  ...
  public String getHostUrl() throws DatabricksParsingException {
    ...
    try {
      ...
      throw new DatabricksParsingException("URI Building failed with exception: " + e.getMessage(), DatabricksDriverErrorCode.CONNECTION_ERROR,null);
    }
  }
  ...
  public String getGcpAuthType() throws DatabricksParsingException {
    ...
    throw new DatabricksParsingException(
        "GCP Auth Type not found. Provide either Google Service Account or Google Credentials file path",  DatabricksDriverErrorCode.CONNECTION_ERROR,null);
  }
  ...
  private IDatabricksComputeResource buildCompute() throws DatabricksSQLException {
    ...
    throw new DatabricksParsingException("Invalid HTTP Path provided " + this.getHttpPath(), DatabricksDriverErrorCode.CONNECTION_ERROR,null);
  }
