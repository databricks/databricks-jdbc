...
/**
 * A wrapper around Apache Arrow's ArrowStreamReader that works without requiring
 * the --add-opens=java.base/java.nio=ALL-UNNAMED JVM flag. This class intercepts and fixes
 * the InaccessibleObjectException that would otherwise occur when Arrow tries to access
 * the protected Buffer.address field.
 */
public class DatabricksArrowStreamReader implements AutoCloseable {
    private static final JdbcLogger LOGGER = JdbcLoggerFactory.getLogger(DatabricksArrowStreamReader.class);
    ...
    private boolean isInaccessibleObjectException(Exception e) {
        ...
        while (cause != null) {
            if (cause.getClass().getName().equals("java.lang.reflect.InaccessibleObjectException") &&
                    ...
                    cause.getMessage().contains("Unable to make field long java.nio.Buffer.address accessible")) {
                return true;
            }
