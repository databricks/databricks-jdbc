...
public abstract class AbstractArrowResultChunk {
  ...
  protected void initializeData(InputStream inputStream)
    ...
    LOGGER.debug(
        String.format(
            "Parsing data for chunk index [%s] and statement [%s]", chunkIndex, statementId));
    recordBatchList = getRecordBatchList(inputStream, rootAllocator, statementId, chunkIndex);
    LOGGER.debug(
        String.format(
            "Data parsed for chunk index [%s] and statement [%s]", chunkIndex, statementId));
  }
   ...
   */
  private List<List<ValueVector>> getRecordBatchList(
