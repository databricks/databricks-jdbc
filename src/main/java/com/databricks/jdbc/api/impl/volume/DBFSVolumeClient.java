...
public class DBFSVolumeClient {
    ...
    private static final JdbcLogger LOGGER =
            JdbcLoggerFactory.getLogger(DBFSVolumeClient.class);
    ...
    private CreateUploadUrlResponse getCreateUploadUrlResponse(String objectPath) throws SQLException {
        LOGGER.debug(
                String.format(
    ...
    public boolean putObject(
        ...
        try{
            CreateUploadUrlResponse response = getCreateUploadUrlResponse(getObjectFullPath(catalog, schema, volume, objectPath));
            System.out.println(response);
            VolumeOperationProcessorDirect volumeOperationProcessorDirect = new VolumeOperationProcessorDirect(response.getUrl(), localPath, response.getHeaders(), connection.getSession());
