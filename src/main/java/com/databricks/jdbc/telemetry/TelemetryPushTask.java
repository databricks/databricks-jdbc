...
class TelemetryPushTask implements Runnable {
    ...
    TelemetryPushTask(List<TelemetryFrontendLog> eventsQueue, boolean isAuthenticated, IDatabricksConnectionContext connectionContext) {
    ...
    public void run() {
        ...
        request.setUploadTime(System.currentTimeMillis()).setProtoLogs(queueToBePushed.isEmpty() ? Optional.empty() : Optional.of(queueToBePushed.stream().map(Object::toString).collect(Collectors.toList())));
