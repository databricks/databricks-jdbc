...
public class HttpRequestTypeBasedRetryHandler {
  ...
  // Context keys for accumulated time tracking
  ...
  private static void initializeRetryState(HttpContext httpContext) {
  ...
  private static void updateAccumulatedTime(
      int statusCode, CloseableHttpResponse response, HttpContext httpContext) {
    int retryInterval = extractRetryInterval(response);
    if (retryInterval <= 0) return;

    if (statusCode == HttpStatus.SC_SERVICE_UNAVAILABLE) {
      long current = (Long) httpContext.getAttribute(TEMP_UNAVAILABLE_ACCUMULATED_TIME_KEY);
      httpContext.setAttribute(TEMP_UNAVAILABLE_ACCUMULATED_TIME_KEY, current + retryInterval);
    } else if (statusCode == HttpStatus.SC_TOO_MANY_REQUESTS) {
      long current = (Long) httpContext.getAttribute(RATE_LIMIT_ACCUMULATED_TIME_KEY);
      httpContext.setAttribute(RATE_LIMIT_ACCUMULATED_TIME_KEY, current + retryInterval);
    }
  }
