...
public enum DatabricksDriverProperty {
    ...
    LOG_PATH("logpath", "Path to log files",  false),
    ...
    USER("user", "Database username", true),
    PASSWORD("password", "Database password",  true),
    CLIENT_ID("OAuth2ClientId", "OAuth2 client ID",  false),
    CLIENT_SECRET("OAuth2Secret", "OAuth2 client secret",  false), // required when authmech=11 and auth_flow!=0
    ...
    OAUTH_REFRESH_TOKEN("OAuthRefreshToken", "OAuth refresh token", false),     // if authmech=11, authflow=0 and you want to use refresh token
    ...
    HTTP_PATH("httppath", "HTTP path for the database endpoint", true),
    ...
    JWT_PASS_PHRASE("Auth_JWT_Key_Passphrase", "Passphrase for JWT key", false),    // if usejwtassertion is true
    ...
    TEMPORARILY_UNAVAILABLE_RETRY("TemporarilyUnavailableRetry", "Retry if temporarily unavailable",  false),
    TEMPORARILY_UNAVAILABLE_RETRY_TIMEOUT("TemporarilyUnavailableRetryTimeout", "Timeout for temporary retry",  false),
    RATE_LIMIT_RETRY("RateLimitRetry", "Enable rate limit retry",  false),
    IDLE_HTTP_CONNECTION_EXPIRY("IdleHttpConnectionExpiry", "Expiry time for idle HTTP connections",  false),
    SUPPORT_MANY_PARAMETERS("supportManyParameters", "Support for many parameters",  false),
    CLOUD_FETCH_THREAD_POOL_SIZE("cloudFetchThreadPoolSize", "Size of thread pool for cloud fetch",  false),
    TOKEN_ENDPOINT("OAuth2TokenEndpoint", "Token endpoint for OAuth", true),
    AUTH_ENDPOINT("OAuth2AuthorizationEndPoint", "Authorization endpoint for OAuth", true),
    ...
    CHECK_CERTIFICATE_REVOCATION("CheckCertificateRevocation", "Enable certificate revocation check",  false),
    ACCEPT_UNDETERMINED_REVOCATION("AcceptUndeterminedRevocation", "Accept undetermined certificate revocation",  false),
    ...
    public static DatabricksDriverProperty[] getRequiredProperties() {
        return Arrays.stream(values())
                .filter(DatabricksDriverProperty::isRequired)
                .toArray(DatabricksDriverProperty[]::new);
    }
}
