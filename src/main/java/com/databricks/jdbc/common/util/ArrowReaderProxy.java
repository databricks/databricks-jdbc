 ...
 * --add-opens=java.base/java.nio=ALL-UNNAMED JVM flag. This is achieved by dynamically
 * determining at runtime whether the JVM environment allows direct access to Buffer.address.
 */
public class ArrowReaderProxy {
    ...
    /**
     * Checks whether the JVM environment allows direct access to Buffer.address without
     * requiring the --add-opens flag. This is determined by doing a test access.
     *
     ...
     * --add-opens=java.base/java.nio=ALL-UNNAMED set. If the flag is not set,
     * this method will use a workaround.
     *
    ...
    public static ArrowStreamReader createReader(InputStream inputStream, BufferAllocator allocator) {
        ...
        try {
            ...
            // Fall back to creating a reader that will throw an exception if it attempts to access Buffer.address
            return new ArrowStreamReader(inputStream, allocator);
        }
    }
    
    /**
     * Creates a safe ArrowStreamReader implementation that works around the Buffer.address
     * access issue by dynamically creating a proxy that intercepts access attempts.
     *
     * @param inputStream The input stream to read Arrow data from
     * @param allocator The BufferAllocator to use
     * @return An ArrowStreamReader instance
     */
    private static ArrowStreamReader createSafeArrowReader(InputStream inputStream, BufferAllocator allocator) {
        ...
        return (ArrowStreamReader) Proxy.newProxyInstance(
                    ...
                    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                        try {
                            ...
                            if (isBufferAddressAccessException(e)) {
                                LOGGER.debug("Caught InaccessibleObjectException from Arrow. Trying workaround.");
                                
                                // If we've already tried reflection and it didn't work, don't try again
                                if (triedReflection) {
                                    throw new IOException("Cannot use Arrow without --add-opens flag. " +
                                            "Please update to latest JDBC driver version that includes a fix for this issue.", e);
                                }
                                ...
                                if ("getVectorSchemaRoot".equals(method.getName())) {
                                    ...
                                    throw new IOException("Arrow requires direct access to java.nio.Buffer.address. " +
