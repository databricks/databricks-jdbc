...
/**
 * A utility class that provides access to the memory address of direct ByteBuffers
 * without requiring reflective access to the protected Buffer.address field.
 * 
 * This approach uses the sun.misc.Unsafe API to directly access the memory address.
 */
public class UnsafeDirectBufferUtility {
    private static final JdbcLogger LOGGER = JdbcLoggerFactory.getLogger(UnsafeDirectBufferUtility.class);
    ...
    static {
        ...
        try {
            ...
            Class<?> unsafeClass = AccessController.doPrivileged(
            ...
            if (unsafeClass != null) {
                ...
                Field theUnsafeField = AccessController.doPrivileged(
                ...
                if (theUnsafeField != null) {
                    ...
                    getInt = AccessController.doPrivileged(
                    ...
                    getObject = AccessController.doPrivileged(
                            (PrivilegedAction<Method>) () -> {
                                try {
                    ...
                    Field addressField = AccessController.doPrivileged(
                            (PrivilegedAction<Field>) () -> {
                                try {
    ...
    /**
     * Gets the memory address of a direct ByteBuffer without requiring reflective
     * access to the protected Buffer.address field.
     * 
     * @param buffer The ByteBuffer to get the address of (must be direct)
     * @return The memory address as a long
     * @throws IllegalArgumentException If the buffer is not direct or if the address cannot be obtained
     */
