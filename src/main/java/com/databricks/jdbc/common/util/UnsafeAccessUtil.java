...
/**
 * Utility class to provide safe access to internal Buffer fields without requiring the
 * --add-opens=java.base/java.nio=ALL-UNNAMED JVM flag.
 *
 * <p>This class uses standard JDK methods to obtain memory addresses of direct byte buffers rather
 * than relying on reflective access to Buffer.address which is not accessible in newer Java
 * versions without --add-opens.
 */
public class UnsafeAccessUtil {
    ...
    try {
      ...
      try {
        getAddress =
            Optional.of(
                lookup.findVirtual(
                    directBufferClass, "address", MethodType.methodType(long.class)));
      ...
      try {
        ...
        cleanerClean =
            Optional.of(
                lookup.findVirtual(cleanerClass, "clean", MethodType.methodType(void.class)));
        ...
        try {
          ...
          cleanerClean =
              Optional.of(
                  lookup.findVirtual(sunCleanerClass, "clean", MethodType.methodType(void.class)));
  ...
  /**
   * Gets the memory address of a direct byte buffer without requiring reflective access to
   * protected fields.
   *
  ...
  /**
   * Checks if the current JVM has direct buffer address access via a non-reflection method or
   * requires the --add-opens flag for direct access.
   *
