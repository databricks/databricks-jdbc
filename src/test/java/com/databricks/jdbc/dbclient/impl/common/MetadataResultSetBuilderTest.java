...
public class MetadataResultSetBuilderTest {
  ...
  @Test
  void testThriftNativeMetadataResultsBypassTransformation() throws SQLException {
    assertThriftNativeMetadataResultIsReturned(
        resultSet -> metadataResultSetBuilder.getFunctionsResult(resultSet, "catalog"));
    assertThriftNativeMetadataResultIsReturned(metadataResultSetBuilder::getProceduresResult);
    assertThriftNativeMetadataResultIsReturned(metadataResultSetBuilder::getProcedureColumnsResult);
    assertThriftNativeMetadataResultIsReturned(metadataResultSetBuilder::getCatalogsResult);
    assertThriftNativeMetadataResultIsReturned(
    ...
    assertThriftNativeMetadataResultIsReturned(metadataResultSetBuilder::getPrimaryKeysResult);
    assertThriftNativeMetadataResultIsReturned(metadataResultSetBuilder::getImportedKeysResult);
    assertThriftNativeMetadataResultIsReturned(
        resultSet ->
  ...
  void testThriftNativeProcedureMetadataIsDetected(
    ...
    assertTrue(
        MetadataResultSetBuilder.matchesThriftNativeMetadataSchema(manifest, operationType));
  }
  ...
  private static Stream<Arguments> procedureMetadataOperations() {
    return Stream.of(
        ...
        Arguments.of(
            MetadataOperationType.GET_PROCEDURE_COLUMNS, PROCEDURE_COLUMNS_COLUMNS));
  }

  private static ResultManifest manifestWithColumns(List<ColumnInfo> columns) {
    return new ResultManifest()
        .setSchema(
            new ResultSchema().setColumnCount((long) columns.size()).setColumns(columns));
  }
  ...
  private void assertThriftNativeMetadataResultIsReturned(MetadataResultCall call)
      throws SQLException {
    DatabricksResultSet resultSet = mock(DatabricksResultSet.class);
    when(resultSet.isThriftNativeMetadataResult()).thenReturn(true);
    assertSame(resultSet, call.execute(resultSet));
