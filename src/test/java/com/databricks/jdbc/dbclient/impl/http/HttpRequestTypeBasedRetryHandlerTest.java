...
public class HttpRequestTypeBasedRetryHandlerTest {
  ...
  public void testSuccessfulRequestWithoutRetry() throws Exception {
    ...
    when(mockHttpClient.execute(eq(mockRequest), any(HttpContext.class))).thenReturn(mockResponse);
    ...
    verify(mockHttpClient, times(1)).execute(eq(mockRequest), any(HttpContext.class));
  }
  ...
  public void testIdempotentRequestRetryOnServerError() throws Exception {
    ...
    when(mockHttpClient.execute(eq(mockRequest), any(HttpContext.class)))
    ...
    verify(mockHttpClient, times(3)).execute(eq(mockRequest), any(HttpContext.class));
  ...
  public void testNonIdempotentRequestRetryOn503WithRetryAfter() throws Exception {
    ...
    // Assert
    assertSame(mockResponse, result);
    verify(mockHttpClient, times(2)).execute(eq(mockRequest), any(HttpContext.class));
  ...
  public void testMaxRetriesExceeded() throws Exception {
    ...
    // Assert
    assertNull(result);
    verify(mockHttpClient, times(6))
        .execute(eq(mockRequest), any(HttpContext.class)); // 5 retries + initial
    verify(failureResponse, times(6)).close(); // All responses are closed when max retries exceeded
  }
