...
class DatabricksCommonHttpClientTest {
    ...
    void testConstructorWithoutSSL() {
        // Assertions to ensure the httpClientWithoutSSL was configured without SSL (not easily directly testable without reflection or breaking encapsulation)
        assertNotNull(httpClientWithoutSSL, "HttpClient instance should be created");
    }

    @Test
    void testConstructorWithSSL() {
        // Assertions to ensure the httpClientWithSSL was configured with SSL (similarly challenging to test directly)
    ...
    void testExecuteGetRequest() throws Exception {
        ...
        when(entity.getContent()).thenReturn(new ByteArrayInputStream("response body".getBytes(StandardCharsets.UTF_8)));
        when(mockedResponse.getAllHeaders()).thenReturn(new BasicHeader[]{new BasicHeader("Content-Type", "application/json")});
