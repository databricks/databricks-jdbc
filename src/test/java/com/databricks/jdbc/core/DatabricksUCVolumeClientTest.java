...
public class DatabricksUCVolumeClientTest {
  ...
  public void testObjectExistsCaseSensitivity(String volume, String objectName, boolean caseSensitive, boolean expected)
          throws SQLException {
    DatabricksUCVolumeClient client = new DatabricksUCVolumeClient(connection);
    ...
    when(statement.executeQuery(createListQuery(TEST_CATALOG, TEST_SCHEMA, "abc_volume1"))).thenReturn(resultSet_abc_volume1);
    when(resultSet_abc_volume1.next()).thenReturn(true, false);
    when(resultSet_abc_volume1.getString("name"))
            .thenReturn("aBc_file1");


    assertEquals(expected, client.objectExists(TEST_CATALOG, TEST_SCHEMA, volume, objectName, caseSensitive));
    verify(statement).executeQuery(listFilesSQL);  }
  ...
  public void testObjectExistsVolumeReferencing(String volume, String objectName, boolean caseSensitive, boolean expected)
          throws SQLException {
    DatabricksUCVolumeClient client = new DatabricksUCVolumeClient(connection);
    ...
    if (volume.equals("abc_volume1")) {
      ...
      when(resultSet_abc_volume1.getString("name"))
              .thenReturn("abc_file3","abc_file1");
    }
    else if (volume.equals("abc_volume2")) {
      when(statement.executeQuery(createListQuery(TEST_CATALOG, TEST_SCHEMA, "abc_volume2"))).thenReturn(resultSet_abc_volume2);
      when(resultSet_abc_volume2.next()).thenReturn(true, true, false);
      when(resultSet_abc_volume2.getString("name"))
              .thenReturn("abc_file4","abc_file1");
    }
  ...
  public void testObjectExistsSpecialCharacters(String volume, String objectName, boolean caseSensitive, boolean expected)
          throws SQLException {
    DatabricksUCVolumeClient client = new DatabricksUCVolumeClient(connection);
