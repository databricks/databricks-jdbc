...
## Cross-Driver Reference (Language-Agnostic)

This section extracts the protocol-level details needed to implement statement-level query tags
in **any** Databricks driver (Node.js, Go, Python, etc.). The Java-specific sections above can
be ignored; this section is self-contained.

### Public API Surface

Expose two methods on the statement/cursor object:

- **`setQueryTags(tags: Map<string, string>)`** — sets key-value tags for subsequent executions.
  Passing null/empty clears. Values may be null (key-only tags).
- **`getQueryTags(): Map<string, string> | null`** — returns current tags or null.

### Validation Rules (enforce on set)

| Rule                            | Behavior                                    |
|---------------------------------|---------------------------------------------|
| Max 20 tags per call            | Reject with validation error                |
| Key must be non-null, non-empty | Reject with validation error                |
| Key max 128 characters          | Reject with validation error                |
| Value max 128 characters        | Reject with validation error (null OK)      |
| Key must not contain `,` `:` `-` `/` `=` `.` | Reject with validation error |

### SEA (SQL Exec API) Transport

Add a `query_tags` field to the `ExecuteStatementRequest` JSON body:
...
```json
{
  ...
  "query_tags": [
    ...
    {"key": "dashboard", "value": "abc123"},
    {"key": "debug"}
  ]
}
```

- Field name: `query_tags` (array of objects)
- Each object: `{"key": "<string>", "value": "<string or omitted>"}`
- Omit the `query_tags` field entirely when no tags are set
- Null values: omit the `value` field (not `"value": null`)

### Thrift Transport

Use `confOverlay` (field 3) on `TExecuteStatementReq`:

```
confOverlay = {"query_tags": "team:marketing,dashboard:abc123,debug"}
```

- Key: `"query_tags"` (string constant)
- Value: comma-separated `key:value` pairs; key-only tags omit the `:value` suffix
- Format matches session-level `QUERY_TAGS` connection parameter format
- Do not set `confOverlay` when no tags are present

### Design Principles

1. Tags are set on the statement object, not passed per-execute call (follows JDBC/DBAPI pattern)
2. Validate on set, not on execute — fail fast with clear error messages
3. No driver-side merging with session-level tags — the server handles merge semantics
4. Store tags immutably after validation to prevent external mutation
5. PreparedStatement/parameterized queries inherit the same tag mechanism
