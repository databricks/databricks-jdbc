...
### Fixed
- Fixed `IS_NULLABLE` field returning incorrect value (`YES`) for non-nullable columns in `getColumns` metadata operation. The fix ensures robust type comparison when the thrift server returns the `NULLABLE` column as a `Short`, `Long`, or `String` instead of an `Integer`.
