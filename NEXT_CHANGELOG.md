...
- Added a new config attribute `DATABRICKS_DISABLE_OAUTH_REFRESH_TOKEN` to control whether refresh tokens are requested in OAuth exchanges. By default, the driver does not include the `offline_access` scope. If `offline_access` is explicitly provided by the user, it is preserved and not removed.
